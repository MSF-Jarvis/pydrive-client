...
class DriveApiClient:
    ...
    def __init__(self):
        ...
        gauth.LoadCredentialsFile(
            path.join(path.dirname(path.abspath(__file__)), "mycreds.txt")
        )
    ...
    def list_files(
        self, parent_folder: str = "root", print_to_stdout: bool = False
    ) -> (list, list):
        """
        ...
        try:
            if self.initial_folder is not None:
                while (
                    parent_folder.metadata["id"] != self.initial_folder.metadata["id"]
                ):
...
def main() -> None:
    ...
    parser.add_argument(
        "-l",
        "--list-files",
        help="List the files in your drive",
        type=str,
        const="root",
        nargs="?",
        action="store",
    ...
    parser.add_argument(
        "-p",
        "--parent-folder",
        help="Only for use with with -u/--upload-file, "
    ...
    file_behaviour_group.add_argument(
        "-s",
        "--skip",
        help="Skip existing files while downloading",
        action="store_true",
    )
    file_behaviour_group.add_argument(
        "-f", "--force", help="Re-download existing files", action="store_true"
    )
    ...
    if args.list_files:
        ...
        try:
            client.download_file(
                url_to_gdrive_id(args.download_file), args.skip, args.force
            )
